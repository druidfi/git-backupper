...
workflow_ids=($(run gh api repos/$GH_OWNER/$GH_REPO/actions/workflows | jq '.workflows[] | .id'))
...
do
  ...
  run_ids=( $(run gh api repos/$GH_OWNER/$GH_REPO/actions/workflows/$workflow_id/runs --paginate | jq '.workflow_runs[].id') )
